 ...
 *  )} Language
*/
import googleTTS from 'google-tts-api'; // CommonJS
import fs from 'fs';
import Stream from 'stream';

export function base64toBinaryStream(base64Text){
...
*/
export function downloadFromInfoCallback(stream, text, {lang, slow, host, timeout, splitPunct}) {
...
* @param {string} cfg.splitPunct
*/
export function getVoiceStream(text, {lang = 'hu', slow = false, host = 'https://translate.google.com', timeout = 10000, splitPunct} = {}) {
...
* @param {string} cfg.splitPunct
*/
export function saveToFile(filePath, text, {lang = 'en-GB', slow = false, host, timeout, splitPunct} = {}) {
 const stream = new Stream.PassThrough();
